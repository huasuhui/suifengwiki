...
<body>
	...
	<c:if test="${param.page=='articleEdit' }">
		<script>
							...
							onImageUpload : function(files) {
								...
								$.ajax({
									...
									success : function(data) {
										$("#summernote").summernote(
												'insertImage', data,
												'image name'); // the insertImage API  
									},
									error : function(data) {
										$("#summernote").summernote(
												'insertImage', data,
			...
			function save() {
				$("#content[value]").val(
						$('#summernote').summernote('code').replace(
								/(^\s*)|(\s*$)/g, ""));
			...
			function check() {
				if ($("#theme[value]").val() == null
						|| $("#theme[value]").val() == "") {
					alert("文章标题不能为空！");
					return false;
				}

				if ($("#content[value]").val() == null
						|| $("#content[value]").val() == "") {
					alert("文章内容不能为空！");
					return false;
				}

				if ($('.articleKindId:checked').attr("value") == null
						|| $('.articleKindId:checked').attr("value") == "") {
					alert("文章分类不能为空！");
					return false;
				}

				if ($("#articleTag[value]").val() == null
						|| $("#articleTag[value]").val() == "") {
					alert("文章标签不能为空！");
					return false;
				}
			...
			function addArticleTag() {
				$("#articleTagStrong").html(
						"本文标签：<a>" + $("#articleTagInput").val() + "</a>");
		...
		<script type="text/javascript">
			...
			function kinddelete() {
				$.ajax({
					...
					type : 'POST',
					data : {
						articleKindId : $("#articleKindId[value]").val(),
						articleKindName : $("#articleKindName[value]").val()
					},
					success : function(data) {
						try {
	...
	<script type="text/javascript">
		...
		function article_draft_save() {
			...
			if (check()) {
				$.ajax({
					...
					type : 'POST',
					data : {
						articleId : $("#articleId[value]").val(),
						theme : $("#theme[value]").val(),
						content : $('#summernote').summernote('code').replace(
								/(^\s*)|(\s*$)/g, ""),
						articleKindId : $("#articleKindId[value]").val(),
						articleTag : $("#articleTag[value]").val(),
						state : 1
					},
					success : function(data) {
		...
		function predetail() {
			...
			if (check()) {
				...
				var content = $('#summernote').summernote('code').replace(
						/(^\s*)|(\s*$)/g, "");
		...
		function setupSubmit() {
			...
			$.ajax({
				...
				type : 'POST',
				data : {
					webSiteTheme : $("#webSiteTheme").val(),
					webSiteEmail : $("#webSiteEmail").val(),
					webSiteDateFormat : $("#webSiteDateFormat").val(),
					webSiteTimeFormat : $("#webSiteTimeFormat").val(),
